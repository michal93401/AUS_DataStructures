...
namespace structures
{
	...
	T PriorityQueueTwoLists<T>::pop()
	{
		...
		if (shortList_->isEmpty() && !longList_->isEmpty()) {
			int size = (int)std::sqrt(longList_->size());
			if (size < 4) {
