...
namespace structures
{
	...
	inline Structure& BinarySearchTree<K, T>::assign(Structure& other)
	{
		if (this != &other) {
			...
			//binarne stromy prosÌm neprech·dzaù for each pri vkladanÌ!
			auto iteratorBegin = new Tree<TableItem<K, T>*>::PreOrderTreeIterator(otherTree.binaryTree_->getRoot());
			...
			/*
				Tree<TableItem<K, T>*>::PreOrderTreeIterator iteratorB(otherTree.binaryTree_->getRoot());
				Tree<TableItem<K, T>*>::PreOrderTreeIterator iteratorE(otherTree.binaryTree_->getRoot());
				...
				//delete netreba v tomto prÌpade
			*/
	...
	inline typename BinarySearchTree<K, T>::BSTTreeNode* BinarySearchTree<K, T>::findBSTNode(K key, bool& found)
	{
		...
		if (binaryTree_->getRoot() == nullptr) {
			...
			while (lastItem->accessData()->getKey() != key) {
	...
	inline void BinarySearchTree<K, T>::extractNode(BSTTreeNode* node)
	{
		...
		////node->isRoot() ? return : nullptr;
		//if (node->isLeaf()) { //!node->hasLeftSon && !node->hasRightSon
		//	if (node->isRoot()) {
		...
		//		node->getParent()->changeLeftSon(node->getRightSon());
		//	}
		//	else {
		//		node->getParent()->changeRightSon(node->getRightSon());
		//	}
		...
		//	} else if (node->isLeftSon()) {
		//		node->getParent()->changeLeftSon(node->getLeftSon());
		//	}
		//	else {
		//		node->getParent()->changeRightSon(node->getLeftSon());
		//		
		//	}
		...
		//}
		//if (node->hasRightSon() && node->hasLeftSon()) {
		//	auto nasledovnik = node->getRightSon();
		//	for (nasledovnik; nasledovnik->hasLeftSon() == true; nasledovnik = nasledovnik->getLeftSon()) {
		...
		//	nasledovnik->getParent()->changeLeftSon(nullptr);
		//	//nasledovnik->setParent(node->getParent());
		//	nasledovnik->changeRightSon(node->getRightSon());
		//	nasledovnik->changeLeftSon(node->getLeftSon());

		//	//node->getRightSon()->setParent(nasledovnik);
		//	//node->getLeftSon()->setParent(nasledovnik);
		//	node->resetParent();
		//	node->changeRightSon(nullptr);
		//	node->changeLeftSon(nullptr);
		//}
